...
- 用户已确认 ribbon 打开中央 Workbench 标签页，插件在标签页内拥有本地左导航，右侧边栏以后只作为可选快速助手。
- `DESIGN.md` 以 Trend Radar 已验证的 Obsidian 原生工作台规则为首要实现参考；WorkBuddy 截图只提供信息架构参考，不作为品牌或功能真相。
- 当前实现只开放概览和运行状态；未来模块保持禁用“规划中”，没有引入公共模块注册器或数据能力。
- 快速助手当前只是真实空容器，不读取 Vault，不提供聊天输入、发送、停止、模型选择或会话历史。
- 自动测试和 CI 覆盖自检已经更新；隔离 Vault 中的中央标签页、导航切换、真实健康检查、按需快速助手、宽窄布局、深浅主题和零错误检查均已通过。远端 CI 和原始 annotations 仍待提交后确认。
