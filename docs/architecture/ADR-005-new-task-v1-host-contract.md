...
- 当前正式 bridge 与健康检查统一精确锁定 DSH `0.1.2-alpha.3`。R1-M 已重新执行公开控制面、Windows 真实 bridge、专用 Vault 健康检查/真实无工具对话和零残留门；R2 已实现精确 session 读取/恢复、Vault 外最小任务索引和启动恢复投影，本地真实 DSH 跨进程门通过，远端 CI 与隔离 Vault 仍按各自门完成。项目/最近 UI、下一批统一运行与发布动作不包含在 R2 中。
