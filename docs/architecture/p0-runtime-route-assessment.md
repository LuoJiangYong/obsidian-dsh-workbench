...
- 评估日期：2026-08-21；正式 bridge 候选复核：2026-08-24
- 目标 DSH：`0.1.1-rc.1`
...
上表只描述 Batch 0B 已实现的只读健康检查，不是正式 bridge 的兼容矩阵。

## 正式 bridge 最新预发布策略

2026-08-24 复核时，DeepSeek Harness 官方 GitHub 最新预发布与 npm `@deepseek-ai/dsh` 的 `latest`/`next` dist-tag 均为 `0.1.1-rc.2`，GitHub tag 指向提交 `b150a551b8d465e31e418e1b2eaf5e79bbb7d28e`。因此下一次获批 bridge 实现批次的当前候选是 `0.1.1-rc.2`；它仍是`仅规划`，不改变现有健康检查只支持 `0.1.1-rc.1` 的能力真相。

每个 bridge 实现或兼容批次必须重新读取当时的 GitHub 最新预发布和 npm dist-tag。二者一致后形成“待验证候选”，再经过上游差异审计、精确版本/tag/commit/lockfile 固定、协议与假 bridge 测试、Windows 真实运行时和清理证据、必要的隔离 Vault 验收以及用户明确批准，才能进入受支持矩阵。

后续可规划定时只读监测并自动生成 issue、兼容提案或 draft PR，但禁止自动修改用户的 DSH 安装、自动合并兼容结论、自动发布插件或自动提交社区目录。新版本出现不等于已经兼容，也不得静默删除当前受支持版本。
