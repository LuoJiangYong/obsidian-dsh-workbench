...
[bridge 协议 v1](./bridge-protocol-v1.md)已固定精确握手、session/turn/seq、一次性权限、取消确认、shutdown、EOF 与单终态；当前实现只通过假 bridge，不是正式 DSH 运行证据。
...
- 协议与假 bridge 已覆盖握手漂移、乱序、超时、取消、权限和 EOF；生产 bridge 进入前仍必须补真实 rc.2 握手、关闭和 Windows 进程测试。
...
Batch 3 只把本 ADR 推进到“协议与假 bridge 已实现”。只有 Batch 4 进一步提交真实 DSH 验收、Windows 生命周期证据、CI 覆盖和隐私披露，才能表述为“生产 bridge 已实现”。
