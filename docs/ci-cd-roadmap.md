...
当前批次只更新 Ardot v2 与治理契约：新建任务位于导航首位，运行合并概览和运行状态后置于最后，三行品牌固定，快速助手保持可选；Ardot 和目标插件 UI 不显示开发阶段、发布批次或治理审批文案，暂不可用导航统一为浅灰禁用态；新建任务社区首发门继续只存在于治理材料。对应治理测试和 CI 覆盖自检已更新；运行源码、协议、隔离 Vault v2 验收和最终用户 UI 验收均未实施。下一实现批次必须从新建任务的输入、事件、状态、取消、权限和错误契约开始，不能直接进入发布。
